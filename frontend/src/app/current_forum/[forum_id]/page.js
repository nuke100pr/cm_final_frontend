...
export default function ChatApp() {
  ...
  // Set forum ID from params in useEffect, not directly in component body
  useEffect(() => {
    if (forum_id) {
      setFid(forum_id);
    }
  ...
  const fetchMessages = async (forumId, page = 1, limit = 1000) => {
    if (!forumId) return; // Don't fetch if no forum ID is available
  ...
  useEffect(() => {
    const organizeMessages = async () => {
      ...
      // Don't automatically set all messages with replies to expanded state
      // Only set initial state for messages that don't have a state yet
      setExpandedReplies(prev => {
        const expandedState = {...prev};
        messages.forEach((msg) => {
          // Only set initial state if it's not already defined
          if (msg.replies && msg.replies.length > 0 && expandedState[msg.id] === undefined) {
            expandedState[msg.id] = false; // Default to collapsed
          }
        });
  ...
  const sendMessage = async () => {
    ...
    if (!fid) {
      console.error("No forum ID available");
      return;
    }
  ...
  // Update poll vote
  const handleVote = async (messageId, optionIndex, isReply = false, parentMessageId = null) => {
    try {
      const response = await fetch(
      ...
      // Update only the specific message without triggering a full re-render
      setMessages(prevMessages => 
        prevMessages.map(message => {
          if (message._id === updatedMessage._id) {
            return updatedMessage;
          } else if (isReply && parentMessageId && message._id === parentMessageId) {
  ...
  // Fetch messages on component mount or when fid changes
  useEffect(() => {
    if (fid) {
      fetchMessages(fid);
    }
  ...
  useEffect(() => {
    // Only auto-scroll when a new message is added, not on every re-render
    if (endOfMessagesRef.current && 
        messages.length > 0 && 
        messages[messages.length - 1].timestamp && 
        new Date(messages[messages.length - 1].timestamp).getTime() > Date.now() - 2000) {
  ...
  return (
    <Box
      sx={{
        ...
        height: "100vh",
        maxWidth: "100vw", // Increased from sm to md for wider chat panel
        mx: "auto",
        width: "100%",
      }}
    >
                    ...
                    <>
                      ...
                      {msg.replies && msg.replies.length > 0 && (
                        <Box sx={{ mt: 1 }}>
                          ...
                          <Collapse in={expandedReplies[msg.id] === true}>
                            <List disablePadding>
